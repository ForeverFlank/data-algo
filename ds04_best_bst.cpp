...
void gen_node(int n, int level, int idx, int *arr)
{
    arr[idx] = n;
    if (n % 2 == 1) return;
    level--;
    gen_node(n - (1 << level), level, 2 * idx + 1, arr);
    gen_node(n + (1 << level), level, 2 * idx + 2, arr);
}
...
void gen_best_bst(int n, CP::map_bst<int, int> &bst)
{
    int k = log2((n + 1) / 2);
    int arr[n];
    gen_node((n + 1) / 2, k, 0, arr);
    for (int &x : arr) bst[x] = 0;
}
